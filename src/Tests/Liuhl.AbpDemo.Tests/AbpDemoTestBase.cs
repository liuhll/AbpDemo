...
using Liuhl.AbpDemo.Migrations.SeedData;
...
namespace Liuhl.AbpDemo.Tests
{
    public abstract class AbpDemoTestBase : AbpIntegratedTestBase<AbpDemoTestModule>
    {
        ...
        protected Task UsingDbContextAsync(Action<AbpDemoDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<AbpDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpDemoDbContext>())
